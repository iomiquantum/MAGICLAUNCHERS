...
if tmux has-session -t "$NAME" 2>/dev/null; then
    ...
    exec tmux new -s "$NAME" "$CAFFEINATE claude --model claude-opus-4-6 --name $NAME --append-system-prompt-file $STATE/orquestador-prompt.txt --dangerously-skip-permissions --rc"
fi
